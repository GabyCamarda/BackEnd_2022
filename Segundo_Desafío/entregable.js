...
class Contenedor {
    ...
    getAll(){
        console.log("OBTENIENDO LOS OBJETOS...");
        array = JSON.parse(fs.readFileSync(this.nombre, "utf-8"));
        showArray(array);
    };

    getById(id){
        console.log("OBTENIENDO EL OBJETO POR ID..." + id);
    ...
    deleteById(id){
        console.log("BORRANDO EL OBJETO POR ID..." + id);
        array = JSON.parse(fs.readFileSync(this.nombre, "utf-8")).filter(x => x.id == id);
...
contenedor.save({ nombre: "Gabriel", apellido: "Camarda" });
contenedor.save({ nombre: "Agustin", apellido: "Meliendrez" });
contenedor.save({ nombre: "Pamela", apellido: "Pereyra" });
contenedor.save({ nombre: "Pedro", apellido: "Rojas" });
